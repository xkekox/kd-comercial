...
const PUBLIC_PRICE_TABLE = {
    ...
    pizzas: {
        ...
        'Pizza 40cm Quadrada': { 100: 2.40, 250: 2.33, 500: 2.26, 1000: 2.20 }
    },
    especiais: {
        'Caixa Peixe': { 100: 2.00, 250: 2.00, 500: 2.00, 1000: 2.00 },
...
const QUOTE_TABLE = Object.fromEntries(
    Object.entries(PUBLIC_PRICE_TABLE).map(([categoryKey, models]) => [
        categoryKey,
        {
            ...
            models: Object.fromEntries(
                Object.entries(models).map(([modelKey, tiers]) => [
                    modelKey,
                    Object.fromEntries(Object.entries(tiers).map(([tierKey, publicPrice]) => [tierKey, publicPrice]))
                ])
            )
        }
    ])
...
function normalizeItem(rawItem) {
    ...
    const tier = getTierBase(quantity);
...
function buildQuote() {
    ...
    const apply250PlusBenefits = elements.freeFreightManaus.checked;
    const shouldChargeScreenFee = hasBoxes && chargeScreenFee && !hasExistingScreen && !apply250PlusBenefits;
    const screenFee = shouldChargeScreenFee ? SCREEN_FEE : 0;
    const deliveryMode = elements.deliveryMode.value;
    const freeFreightManaus = apply250PlusBenefits;
    const manualFreight = Number(elements.freightValue.value) || 0;
    const hasManualFreight = manualFreight > 0;
    const freight = deliveryMode === 'retirada'
        ? 0
        : (hasManualFreight ? manualFreight : (freeFreightManaus ? 0 : 0));
    const total = Number((subtotal + screenFee + freight).toFixed(2));
    const autoSupplierCost = Number(items.reduce((sum, item) => sum + item.supplierSubtotal, 0).toFixed(2));
    const freightLabel = deliveryMode === 'retirada'
        ? 'retirada pelo cliente'
        : (freight > 0 ? `${money(freight)} (manual)` : (freeFreightManaus && totalQuantity >= 250 ? 'gratis' : 'nao informado'));
    ...
    } else if (hasBoxes && apply250PlusBenefits) {
        screenLabel = 'gratis via condicao 250+';
    }
...
function sendCurrentQuoteWhatsapp() {
    if (!state.lastQuote?.text) {
        alert('Gere o orcamento antes de mandar para o cliente.');
        return;
    }
    ...
    const bodyHtml = `
        <div class="topbar">
            <div>
                ...
                <div class="muted">Ordem de producao para fornecedor</div>
            </div>
            <div>
                <div class="title">Pedido para producao</div>
        ...
        <div class="card">
            <div class="grid">
                ...
                <div>
                    ...
                    <div class="value">${escapeHtml(deliveryText)}</div>
                </div>
                <div>
        ...
        <div class="card">
            ...
            <div class="note">${escapeHtml(observations || 'Produzir conforme pedido acima.')}</div>
        </div>
    `;

    openPdfDocument(
        'Pedido fornecedor KD Embalagens',
        `pedido-fornecedor-${safeFileName(customerName)}.pdf`,
        bodyHtml
    );
}
...
function sendChargeWhatsapp() {
    const quote = state.lastQuote;
    if (!quote) {
        alert('Gere o orcamento antes de cobrar o cliente.');
        return;
    }
    ...
    if (pending <= 0) {
        alert('Nao ha saldo pendente para cobrar deste cliente.');
        return;
    }
    const message = [
...
function generateReceiptText() {
    ...
    return [
        ...
        `Forma de pagamento: ${getPaymentMethodLabel(elements.paymentMethod.value)}`,
...
function generateReceipt() {
    if (!state.lastQuote) {
        alert('Gere o orcamento antes de gerar o recibo.');
        return;
    }

    const receivedAmount = Number(elements.receivedAmount.value) || 0;
    if (receivedAmount <= 0) {
        alert('Preencha o valor recebido para gerar o recibo.');
        return;
    }

    const text = generateReceiptText();
    elements.receiptOutput.textContent = text;
    elements.receiptOutput.scrollIntoView({ behavior: 'smooth', block: 'start' });
...
<body>
  ...
  <script>
    ...
    if (window.KDAndroid && window.KDAndroid.printHtml) {
        window.KDAndroid.printHtml(fileName, doc);
        return;
    }
...
function downloadQuotePdf() {
    const quote = state.lastQuote;
    if (!quote) {
        alert('Gere o orcamento antes de abrir o PDF.');
        return;
    }
...
function downloadReceiptPdf() {
    ...
    const receivedAmount = Number(elements.receivedAmount.value) || 0;
    if (!quote) {
        alert('Gere o orcamento antes de abrir o recibo.');
        return;
    }
    if (receivedAmount <= 0) {
        alert('Preencha o valor recebido para gerar o recibo.');
        ...
        <div class="card">
            <div class="grid">
                ...
                <div>
                    ...
                    <div class="value">${escapeHtml(getPaymentMethodLabel(elements.paymentMethod.value))}</div>
                </div>
                <div>
...
elements.freightValue.addEventListener('input', () => {
    const manualFreight = Number(elements.freightValue.value) || 0;
    if (manualFreight > 0 && elements.deliveryMode.value !== 'entrega') {
        elements.deliveryMode.value = 'entrega';
        syncDeliveryModeUI();
    }
    refreshQuotePreview();
});
...
elements.generateQuote.addEventListener('click', () => {
    ...
    syncCustomerPaymentUI();
    elements.quoteOutput.textContent = quote.text;
    elements.quoteOutput.scrollIntoView({ behavior: 'smooth', block: 'start' });
});
